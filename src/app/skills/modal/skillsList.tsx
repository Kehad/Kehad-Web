...
import nextjs from "@/assets/nextIcon.png";
...
export const itemList = [
        ...
        >
          ...
          <defs>
            <clipPath id="prefix__clip0">
              <path fill="#07c514" d="M0 0h54v32.4H0z" />
            </clipPath>
          </defs>
        </svg>
      ),
    ...
    },
    {
      ...
      link: (
        <img
          className="h-9 sm:h-9 md:h-11 object-contain"
          alt="NodeJS"
          src={nodeIcon.src}
        />
      ),
...
const SkillsList = function (props: any) {
  return (
    <div className="group/item flex justify-between items-center rounded-2xl w-full py-3 px-4 lg:py-3 lg:px-5 bg-white/40 dark:bg-white/5 border border-white/60 dark:border-white/10 hover:bg-white dark:hover:bg-white/10 hover:border-primary/50 hover:shadow-[0_4px_20px_rgba(7,197,20,0.15)] transition-all duration-300 transform hover:-translate-y-[2px]">
      <p className="font-judson text-lg md:text-xl lg:text-2xl font-bold tracking-wide text-gray-800 dark:text-gray-200 transition-colors group-hover/item:text-primary">
        {props.name}
      </p>
      <div className="flex items-center justify-center p-2 rounded-xl bg-gray-100/30 dark:bg-black/30 group-hover/item:bg-primary/10 transition-all duration-300 shadow-sm border border-transparent group-hover/item:border-primary/20">
        {props.link}
      </div>
    </div>
  );
...
export const frontendSkills = itemList.filter((item) => ["HTML", "CSS", "JS", "React", "Tailwind CSS", "NextJS"].includes(item.name));
export const backendSkills = itemList.filter((item) => ["NodeJS", "ExpressJS"].includes(item.name));
export const otherSkills = itemList.filter((item) => ["React Native", "ElectronJS"].includes(item.name));
