...
const SpotifyPlaylist = () => {
  ...
  // 🔑 STEP 1: Handle redirect with code
  useEffect(() => {
  ...
  useEffect(() => {
    if (token && !currentTrack) {
      toast.info(
        "You are logged in but no current track is playing on Spotify."
      );
    }
  }, [token, currentTrack]);
  ...
  return (
    <div>
      ...
      {!token ? (
        <a
          href={`${AUTH_ENDPOINT}?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${REDIRECT_URI}&scope=playlist-read-private user-read-currently-playing user-read-playback-state`}
          className="bg-[#DFFEE2] dark:text-black text-black hover:text-white p-3 rounded-full border-[#dffee2] border-2 hover:border-2 hover:border-[#dffee2] hover:bg-transparent"
        >
          Login to Spotify
        </a>
